...
if WindUI then
    Window =
        WindUI:CreateWindow(
        {
            Title = "LANDER HUB  |  Block Spin 🔫|",
            Icon = "list",
            Author = "HI! NICE TO MEET YOU :)",
            Folder = "LANDER HUB Now!!!",
...
local function ToggleBring(name)
    ...
    if Active then
        ...
        task.spawn(function()
            while Active do Attachment1.WorldCFrame = targetRoot.CFrame; task.wait() end
        end)
    else
...
local function createESP(player)
    ...
    nameText.Size = 16; nameText.Center = true; nameText.Outline = true
    nameText.Color = isPlayerExcluded(player.Name) and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(255, 255, 255)
    nameText.Font = 4
    local distanceText = Drawing.new("Text")
    distanceText.Size = 14; distanceText.Center = true; distanceText.Outline = true
    distanceText.Color = Color3.fromRGB(255, 255, 255); distanceText.Font = 4
    local healthBg = Drawing.new("Square")
    healthBg.Filled = false; healthBg.Thickness = 1; healthBg.Color = Color3.fromRGB(0, 0, 0)
    healthBg.Transparency = 0.9; healthBg.Visible = false
    local healthFg = Drawing.new("Square")
    healthFg.Filled = true; healthFg.Transparency = 0.9; healthFg.Visible = false
    ...
    local conn = RunService.RenderStepped:Connect(function()
        ...
        if healthESPEnabled and humanoid and humanoid.Health > 0 then
            ...
            local barHeight = 4; local barWidth = 60; local healthX = centerX - barWidth / 2
            healthBg.Position = Vector2.new(healthX, currentTopY - barHeight - 2)
            healthBg.Size = Vector2.new(barWidth, barHeight); healthBg.Visible = true
            healthFg.Position = Vector2.new(healthX, currentTopY - barHeight - 2)
            healthFg.Size = Vector2.new(barWidth * perc, barHeight)
            healthFg.Color = Color3.fromHSV(perc * 0.333, 0.8, 0.9); healthFg.Visible = true
        ...
        if nameESPEnabled then
            ...
            nameText.Text = player.Name; nameText.Size = math.floor(dynamicSize)
            nameText.Color = isPlayerExcluded(player.Name) and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(255, 255, 255)
            nameText.Outline = true
            nameText.Position = Vector2.new(centerX, currentTopY - 16); nameText.Visible = true
...
local function loadESP()
    ...
    Players.PlayerAdded:Connect(function(player)
        if player ~= LocalPlayer then
            ...
            if player.Character and not espPlayers[player] then task.wait(0.1); createESP(player) end
        end
    end)
...
if not isMobile then
    ...
    SilentFOVCircle.Thickness = 1.4; SilentFOVCircle.NumSides = 64
    SilentFOVCircle.Filled = false; SilentFOVCircle.Transparency = 3
    SilentFOVCircle.Radius = FOVRadius; SilentFOVCircle.Visible = false
else
    local ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = "MobileFOV"; ScreenGui.Parent = LocalPlayer:WaitForChild("PlayerGui")
    ...
    local circleUI = Instance.new("UICorner"); circleUI.CornerRadius = UDim.new(1, 0); circleUI.Parent = SilentFOVCircle
    local border = Instance.new("UIStroke"); border.Color = Color3.fromRGB(255, 255, 255)
    border.Thickness = 2; border.Transparency = 0.2; border.Parent = SilentFOVCircle
...
local function calculateVelocity(player)
    ...
    local totalVel = Vector3.new(); local count = 0
    for i = 2, #hist do
        local dt = hist[i].time - hist[i - 1].time
        if dt > 0 then totalVel = totalVel + (hist[i].pos - hist[i - 1].pos) / dt; count = count + 1 end
    end
            ...
            if SilentAimEnabled and args[2] == "shoot_gun" and CurrentTarget then
                ...
                if head and hrp and humanoid then
                    ...
                    local success, beam = pcall(function()
                        ...
                        part.Material = Enum.Material.Neon; part.Transparency = 0.35
                        part.Color = Color3.fromRGB(255, 0, 0); part.Parent = workspace
                        Debris:AddItem(part, 4); return part
                    end)
                    if humanoid then
                            ...
                            if humanoid and humanoid.Health < previousHealth then
                                ...
                                for _, part in ipairs(CurrentTarget.Character:GetDescendants()) do
                                    if part:IsA("BasePart") then
                                        ...
                                        box.Material = Enum.Material.Neon; box.Color = Color3.fromRGB(255,0,0)
                                ...
                                if head then
                                    ...
                                    blood.Size = Vector3.new(0.2, 0.2, 0.2); blood.Shape = Enum.PartType.Ball
                                    blood.Material = Enum.Material.Neon; blood.Color = Color3.fromRGB(255,0,0)
                                    blood.CFrame = CFrame.new(head.Position); blood.Anchored = false; blood.CanCollide = false
                                    blood.Parent = Workspace
                                    local bv = Instance.new("BodyVelocity")
                                    bv.Velocity = Vector3.new(math.random(-5,5), math.random(5,10), math.random(-5,5))
                                    bv.P = 5000; bv.MaxForce = Vector3.new(4000,4000,4000); bv.Parent = blood
        ...
        if TracerTarget and TracerTarget.Character then
            ...
            if humanoid and humanoid.Health > 0 and targetPart then
                ...
                if targetOnScreen then
                    Tracer.Visible = true; Tracer.From = centerScreenPos
                    Tracer.To = Vector2.new(targetScreenPos.X, targetScreenPos.Y)
                    Tracer.Color = Color3.fromRGB(255, 50, 50); Tracer.Thickness = 1.3
                    if not TracerESP then
                        TracerESP = {}
                        for i = 1, 4 do
                            TracerESP[i] = Drawing.new("Line")
                            TracerESP[i].Color = Color3.fromRGB(255, 255, 255)
                            TracerESP[i].Thickness = 1.2; TracerESP[i].Visible = true
                        end
                    end
                    ...
                    local sizeX = sizeY
                    sizeX = math.clamp(sizeX, 8, 25); sizeY = math.clamp(sizeY, 8, 25)
                    local top = Vector2.new(center.X, center.Y - sizeY); local bottom = Vector2.new(center.X, center.Y + sizeY)
                    local left = Vector2.new(center.X - sizeX, center.Y); local right = Vector2.new(center.X + sizeX, center.Y)
...
RunService.Heartbeat:Connect(function()
    ...
    if isDowned() then
        ...
        undergroundBaseCFrame = nil; flickering = false
    end
end)
RunService.Heartbeat:Connect(function()
...
ContextActionService:BindAction("FlyUp", function(actionName, inputState, inputObject)
    ...
    if isJumpPressed then
        if inputState == Enum.UserInputState.Begin then isFlyingUp = true; Humanoid.Jump = true; return Enum.ContextActionResult.Sink
        elseif inputState == Enum.UserInputState.End then isFlyingUp = false; return Enum.ContextActionResult.Sink end
    end
...
LocalPlayer.CharacterAdded:Connect(function(newChar)
    ...
    lockedY = nil; teleportActive = false; lockYPosition()
end)
lockYPosition()
LocalPlayer.CharacterAdded:Connect(function()
    task.wait(1); running = false; task.wait(0.1); StartAutoAttack()
end)
StartAutoAttack()
...
-- ================================================================
-- DASHBOARD TAB  |  ต้องอยู่ก่อน Tab COMBAT เสมอ
-- ใช้ pattern เดียวกับ Lander Hub | GnB
-- ================================================================
do
    local DashboardTab = Window:Tab({ Title = "Dashboard", Icon = "layout-dashboard" })
    DashboardTab:Select() -- เปิด tab นี้เป็น tab แรก

    -- ── SECTION 1: BANNER ──────────────────────────────────────
    local BannerSection = DashboardTab:Section({
        Title  = "Lander Hub  |  Block Spin 🔫",
        Icon   = "star",
        Opened = true,
    })
    ...
    BannerSection:Paragraph({
        Title = "Lander Hub  v1.0  —  Block Spin 🔫",
        Desc  =
            "Game   : Block Spin\n"  ..
            "Hub    : Lander Hub\n"  ..
            "Exec   : " .. (identifyexecutor and identifyexecutor()
                           or getexecutorname and getexecutorname()
                           or "Unknown") .. "\n" ..
            "Player : " .. game:GetService("Players").LocalPlayer.Name,
        Image     = "https://raw.githubusercontent.com/Cannifin/Image/refs/heads/main/Screenshot_2026-03-15-18-39-03-82_f380df83a0cf994d27820a806b923e1f.jpg",
        ImageSize = 60,
    })
    ...
    local _widgetUrl    = "https://discord.com/api/guilds/1480053363302338610/widget.json"

    local DiscordCard = DiscordSection:Paragraph({
        Title     = "Lander Hub  Discord",
        Desc      = "🟢 Loading...\n👥 Loading...",
        ...
        Buttons   = {
            {
                ...
                Callback = function()
                    ...
                    if WindUI then
                        WindUI:Notify({
                            Title    = "Copied!",
                            Content  = "discord.gg/YF2DNag2Cq — Copied!",
                            Duration = 3,
                            Icon     = "copy",
                        })
                    end
    ...
    -- live Discord stats (online + member count)
    local function _updateDiscordStats()
        local online, members = 0, 0
        pcall(function()
            local d = _HttpSvc:JSONDecode(game:HttpGet(_widgetUrl))
            if type(d) == "table" then online = tonumber(d.presence_count) or 0 end
        end)
        pcall(function()
            local d = _HttpSvc:JSONDecode(
                game:HttpGet("https://discord.com/api/v9/invites/YF2DNag2Cq?with_counts=true")
            )
            if type(d) == "table" and d.guild then
                members = tonumber(d.approximate_member_count)   or members
                online  = tonumber(d.approximate_presence_count) or online
            end
        end)
        pcall(function()
            DiscordCard:SetDesc(string.format("🟢 %d Online\n👥 %d Members", online, members))
        end)
    end

    task.defer(function()
        task.wait(2)
        while true do pcall(_updateDiscordStats); task.wait(60) end
    end)

    -- ── SECTION 3: CHANGELOG ──────────────────────────────────
    local ChangelogSection = DashboardTab:Section({
        Title  = "Changelog",
        Icon   = "file-text",
        Opened = true,
    })
    ...
    ChangelogSection:Paragraph({
        Title = "v1.0  —  Current",
        Desc  =
            "+ Silent Aim — FOV circle, Tracer, Prediction\n"      ..
            "+ Red Line Lock (auto attach target)\n"                ..
            "+ Weapon Mods: Fire Rate, Accuracy, Recoil, Auto\n"   ..
            "+ Melee Aura (WideFists), Auto Attack nearby players\n"..
            "+ ESP: Name, Health, Distance, Inventory Viewer\n"     ..
            "+ Highlight ESP (blue outline)\n"                      ..
            "+ Walk Speed Multiplier, Jump/Float\n"                 ..
            "+ Infinite Stamina (sprint module hook)\n"             ..
            "+ Anti Lock, Anti Kill, Anti Ragdoll\n"                ..
            "+ Item ESP — Rarity color + Highlight\n"               ..
            "+ Pickup Suction (auto collect items)\n"               ..
            "+ Auto Finish, Auto Skip Crate, Auto Respawn\n"        ..
            "+ Server Hop, Server Rejoin by ID\n"                   ..
            "+ Claim All Quest\n"                                   ..
            "+ Snap Under Map (Z key)\n"                            ..
    ...
    local StatsSection = DashboardTab:Section({
        Title  = "Live Stats",
        Icon   = "bar-chart-2",
        Opened = false,
    })

    local StatsCard = StatsSection:Paragraph({
        Title = "Game Info",
        Desc  = "Loading...",
    })
    ...
    task.defer(function()
        ...
        while true do
            pcall(function()
                ...
                local pos     = hrp and string.format(
                    "(%.0f, %.0f, %.0f)", hrp.Position.X, hrp.Position.Y, hrp.Position.Z
                ) or "N/A"
                local hp      = hum and string.format(
                    "%.0f / %.0f", hum.Health, hum.MaxHealth
                ) or "N/A"
                local players = #game:GetService("Players"):GetPlayers()
                StatsCard:SetDesc(string.format(
                    "🏓 Ping  : %dms\n" ..
                    "❤️  HP    : %s\n"  ..
                    "📍 Pos   : %s\n"  ..
                    "👤 Players: %d",
...
local AttachToggle = Tab:Toggle({
    Title = "Red Line Lock", Default = false,
...
local FriendsInput = Tab:Input({
    Title = "Safe Friend", Desc = "", Value = "", InputIcon = "shield-check",
    ...
    Callback = function(input)
        ...
        for _, player in pairs(Players:GetPlayers()) do
            if espPlayers[player] and espPlayers[player].drawings then
                espPlayers[player].drawings[1].Color = isPlayerExcluded(player.Name) and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(255, 255, 255)
            end
        end
    end
})
...
getgenv().FireRateValue = 1000; getgenv().AccuracyValue = 1; getgenv().RecoilValue = 0
getgenv().Durability = 999999999; getgenv().Auto = true; getgenv().automatic = true
getgenv().AutoValue = true; getgenv().GunModsAutoApply = false
...
local function debugPrintAttributes(tool)
    local attrs = tool:GetAttributes(); local keys = {}
...
local function applyGodGun(tool)
    ...
    task.spawn(function()
        for attempt = 1, 20 do
            local attrNames = tool:GetAttributes(); local keys = {}
    ...
    task.wait(0.5); debugPrintAttributes(tool)
end
RunService.Heartbeat:Connect(function()
...
local Automodifyer
Tab_mods:Toggle({
    Title = "Automatic", Icon = "check", Type = "Checkbox", Value = false,
    Callback = function(v)
        getgenv().automatic = v; getgenv().GunModsAutoApply = v
...
local function modifyFists(tool, enable)
    if tool then
        local attributes = tool:GetAttributes(); local keys = {}
...
local function checkAndModifyFists()
    local Character = LocalPlayer.Character; local backpack = LocalPlayer:FindFirstChild("Backpack")
...
myConfig:Register("Fists Modifier", Tab_mods:Toggle({
    Title = "Melee Aura", Desc = "WideFists", Default = false,
    Callback = function(Value) FistsBuffEnabled = Value; checkAndModifyFists() end
}))
local autoAttackToggle = Tab_mods:Toggle({
    Title = "Auto Attack", Default = false,
...
local ItemsESPToggle = Tab_ESP:Toggle({
    Title = "Inventory Viewer", Default = false,
...
myConfig:Register("NameESP", Tab_ESP:Toggle({Title = "Name", Default = false, Callback = function(state) nameESPEnabled = state end}))
myConfig:Register("HealthESP", Tab_ESP:Toggle({Title = "Health", Default = false, Callback = function(state) healthESPEnabled = state end}))
myConfig:Register("DistanceESP", Tab_ESP:Toggle({Title = "Distance", Default = false, Callback = function(state) distanceESPEnabled = state end}))
myConfig:Register("HighlightESP", Tab_ESP:Toggle({
...
function updateHighlight(player)
    ...
    if highlightEnabled then
        ...
        highlight.Name = "PlayerHighlight"; highlight.Adornee = player.Character
        highlight.FillColor = Color3.fromRGB(0, 170, 255); highlight.OutlineColor = Color3.fromRGB(0, 170, 255)
        highlight.Parent = workspace; playerHighlights[player] = highlight
    end
end
...
myConfig:Register("WalkSpeed", Tab_Character:Toggle({Title = "Walk Speed", Default = false, Callback = function(state) walkSpeedEnabled = state end}))
myConfig:Register("SpeedMultiplier", Tab_Character:Slider({Title = "Speed Multiplier", Step = 0.5, Value = {Min = 1, Max = 5, Default = 2}, Callback = function(value) speedValue = value * 0.05 end}))
myConfig:Register("JumpPower", Tab_Character:Toggle({Title = "Jump Power", Default = false, Callback = function(state) FlyEnabled = state; if not FlyEnabled then flying = false end end}))

local Net = {}
function Net.send(...)
    local args = {...}; CounterTable.event = CounterTable.event + 1
    ...
    Callback = function(state)
        ...
        if AutoSprintEnabled then
            ...
            if success and SprintModule then
                ...
                else AutoSprintEnabled = false; AutoSprintToggle:Set(false) end
            else AutoSprintEnabled = false; AutoSprintToggle:Set(false) end
        else
...
myConfig:Register("AntiLock", Tab_Character:Toggle({Title = "Anti Lock", Default = false, Callback = function(state) getgenv().Sky = state; if state then getgenv().SkyAmount = 1500 end end}))
myConfig:Register("AntiKill", Tab_Character:Toggle({
    Title = "Anti Kill", Default = false,
    Callback = function(state)
        enabled = state
        if WindUI then WindUI:Notify({Title = state and " Anti Kill Enabled" or "❌ Anti Kill Disabled", Duration = 3}) end
    end
}))
...
myConfig:Register("PickupItems", Tab_Character:Toggle({Title = "Pickup items", Default = false, Callback = function(state) sucking = state end}))
myConfig:Register("AntiRagdoll", Tab_Character:Toggle({
    ...
    Callback = function(state)
        ...
        pcall(function()
            ...
            local function sendAction(action)
                ...
                ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("Send"):FireServer(CT.event, action)
            end
            task.spawn(function()
                while _AntiRagdollEnabled do
                    sendAction("end_ragdoll_early"); task.wait(0.3)
                    if not _AntiRagdollEnabled then break end
                    sendAction("clear_ragdoll"); task.wait(0.3)
                end
            end)
        end)
    end
}))
myConfig:Register("HideName", Tab_Character:Toggle({
    Title = "Hide Name", Default = false,
    Callback = function(state)
        pcall(function()
...
myConfig:Register("AutoRespawn", Tab_Character:Toggle({
    Title = "Auto Respawn", Default = false,
    Callback = function(state)
            ...
            task.spawn(function()
                while _AutoRespawnEnabled do
                    ...
                    if hum and hum.Health <= 0 then
                        task.wait(6)
                        if _AutoRespawnEnabled then sendAction("death_screen_request_respawn") end
                    end
                    task.wait(0.5)
                end
            end)
...
myConfig:Register("SnapUnderMap", Tab_Character:Toggle({
    Title = "Snap Under Map", Default = false,
    Callback = function(state)
        ...
        if featureEnabled then
            ...
            teleportActive = true; performTeleport()
        else
            teleportActive = false; lockedY = nil; startY = nil
        end
    end
}))
myConfig:Register("SnapHeight", Tab_Character:Slider({
    Title = "Snap:", Step = 1, Value = {Min = 1, Max = 50, Default = 10},
    Callback = function(value)
        ...
        if teleportActive and HumanoidRootPart and startY then
            ...
            HumanoidRootPart.CFrame = CFrame.new(bottomPos); lockedY = bottomPos.Y
        end
    end
}))
...
CorePart.Anchored = true; CorePart.CanCollide = false; CorePart.Transparency = 1
myConfig:Register("AutoFinnish", Tab_player:Toggle({
    Title = "Auto Finnish", Default = false,
    Callback = function(state)
...
local function safeToggle(title, desc, key, callback)
    pcall(function()
        local ToggleElement = Tab_buyer:Toggle({
            Title = title, Desc = desc, Icon = "check", Type = "Checkbox", Default = false,
            Callback = function(state) AutoSkipEnabled = state; if callback then callback(state) end end
        })
...
Tab_misc:Input({
    Title = "Server Hop by ID", Value = "", InputIcon = "send", Type = "Input", Placeholder = "id sever here!",
    Callback = function(input)
...
Tab_misc:Button({
    Title = "Server Rejoin", Desc = "Come back old sever",
    Callback = function()
...
local function FindServer()
    local servers = {}; local cursor = nil
    for i = 1, 5 do
        local success, result = pcall(function()
...
Tab_misc:Button({
    Title = "Server Hop", Desc = "Hop to a new server (sometime don't work)", Locked = false,
    Callback = function()
        local PlaceId = 104715542330896
        local HS = game:GetService("HttpService"); local TS = game:GetService("TeleportService")
        local success, servers = pcall(function()
            return HS:JSONDecode(game:HttpGet("https://games.roblox.com/v1/games/" .. PlaceId .. "/servers/Public?sortOrder=Desc&limit=100"))
        end)
        ...
        TS:TeleportToPlaceInstance(PlaceId, targetServer.id, game.Players.LocalPlayer)
    end
})
Tab_misc:Divider()
    ...
    Callback = function()
        task.spawn(function()
            local success, err = pcall(function()
                ...
                local Net2 = {}
                function Net2.get(...)
                    local args = {...}; CT.func = (CT.func or 0) + 1
                    return ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("Get"):InvokeServer(CT.func, table.unpack(args))
                end
                local questFrame = Players.LocalPlayer:WaitForChild("PlayerGui"):WaitForChild("Quests"):WaitForChild("QuestsHolder"):WaitForChild("QuestsScrollingFrame")
                for _, child in ipairs(questFrame:GetChildren()) do
                    if child:IsA("Frame") or child:IsA("TextButton") or child:IsA("ImageButton") then
                        Net2.get("claim_quest", child.Name); task.wait(0.2)
                    end
                end
            end)
...
myConfig:Register("ClaimAllQuest", Tab_misc:Button({Title = "Claim All Quest (dummy)", Callback = function() end}))

local saveFunc = myConfig["Save"]; local deleteFunc = myConfig["Delete"]; local loadFunc = myConfig["Load"]
Tab_misc:Section({Title = "Config Management"})
myConfig:Register("SaveConfig", Tab_misc:Button({Title = "Save Config", Callback = function() if saveFunc then saveFunc(myConfig) end end}))
myConfig:Register("DeleteConfig", Tab_misc:Button({Title = "Delete Config", Callback = function() if deleteFunc then deleteFunc(myConfig) end end}))
...
Util.tween = function(instance, tweenInfo, properties)
    if instance and instance:IsA("NumberValue") and properties and properties.Value ~= nil then
        instance.Value = properties.Value; return {Cancel = function() end}
    end
...
local function hookButton(button)
    ...
    CoreUI.on_click(button, function()
        ...
        task.wait(0.12); EmotesUI.enabled.set(false)
    end)
...
RunService.RenderStepped:Connect(function()
    ...
    for _, d in pairs(item_drawings) do
        d.circle.Visible = false; d.innerCircle.Visible = false
        d.name.Visible = false; d.amount.Visible = false
    ...
    for i = 1, math.min(20, #nearbyItems) do
        ...
        if not d.highlight or not d.highlight.Parent then
            local highlight = Instance.new("Highlight")
            highlight.Name = "ESP_Highlight"; highlight.FillTransparency = 0.5; highlight.OutlineTransparency = 0.1
            highlight.Adornee = item; highlight.Parent = item; d.highlight = highlight
        end
        ...
        if onScreen then
            ...
            d.amount.Text = amount > 1 and "[" .. tostring(amount) .. "]" or ""; d.amount.Visible = amount > 1
        else
